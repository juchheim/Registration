...
function simple_webhook_test_handler() {
    $log_file_path = __DIR__ . '/simple_webhook_log.txt';
    $log_file = fopen($log_file_path, 'a');
    if ($log_file) {
        fwrite($log_file, "Simple Webhook Handler Triggered at " . date('Y-m-d H:i:s') . "\n");
        fclose($log_file);
    } else {
...
add_action('admin_post_nopriv_simple_webhook_test', 'simple_webhook_test_handler');
add_action('admin_post_simple_webhook_test', 'simple_webhook_test_handler');
